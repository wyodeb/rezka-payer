...
struct DetailedMediaItemView: View {
    ...
    var body: some View {
        ...
        .overlay(overlayView)
        .onChange(of: viewModel.currentTranslationTitle) { newValue in
    ...
    private var detailView: some View {
        ...
        .fullScreenCover(isPresented: $isPlayerPresented, content: {
            PlayerViewController(videoURL: URL(string: viewModel.stream), viewModel: viewModel)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 18) {
                    ...
                    MetadataChip(
                        ...
                        isTranslationMenuPresented = true
                    }

                    if let seasons = viewModel.seasonsInCurrentTranslation, !seasons.isEmpty {
                        MetadataChip(
                            icon: "calendar",
                            caption: "Season",
                            value: viewModel.currentSeasonTitle
                        ) {
                            isSeasonsMenuPresented = true
                        }

                        MetadataChip(
                            icon: "play.tv.fill",
                            caption: "Episode",
                            value: viewModel.currentEpisodeTitle
                        ) {
                            isEpisodesMenuPresented = true
                        }
                    }
                    ...
                    if viewModel.streams?.qualities != nil {
                        MetadataChip(
                            ...
                            caption: "Quality",
                            value: viewModel.currentQuality.rawValue
                        ) {
